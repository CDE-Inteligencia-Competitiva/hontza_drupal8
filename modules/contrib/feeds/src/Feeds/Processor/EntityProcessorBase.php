...
abstract class EntityProcessorBase extends ProcessorBase implements EntityProcessorInterface {
  ...
  public function process(FeedInterface $feed, ItemInterface $item, StateInterface $state) {
    ...
    //intelsat-2016
    $vigilancia_controller=new VigilanciaController();
    if($vigilancia_controller->vigilancia_is_item_duplicado($feed,$item,$state)){
      return;
    }
    ...
    try {
      ...
      $entity->get('field_my_group')->setValue($my_group_gid);      
      // And... Save! We made it.

      //mireia2017
      $field_feed_categories=$feed->get('field_feed_categories')->getValue();
      $entity->get('field_item_canal_category_tid')->setValue($field_feed_categories);

      //mireia2017fuentes
      $field_canal_source_type=$feed->get('field_canal_source_type')->getValue();
      $entity->get('field_item_source_tid')->setValue($field_canal_source_type);
      
      
      $this->storageController->save($entity);
      //intelsat-2016
      //Group::addContent($entity,'group_node-item');
      if(isset($my_group_gid[0]['target_id']) && !empty($my_group_gid[0]['target_id'])){
        $my_group=Group::load($my_group_gid[0]['target_id']);
        $my_plugin_id='group_node:item';
