...
/**
 * @file
 * Contains \Drupal\wiki\Form\WikiSimpleSearchForm.
 */
...
class WikiSimpleSearchForm extends FormBase {
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $triggerdElement=$form_state->getTriggeringElement();
      if($triggerdElement['#name']=='search_btn'){
        ...
        $url = Url::fromRoute('wikis_grupo',array('group'=>$grupo_nid,'search'=>$search));
        $form_state->setRedirectUrl($url);
   
      }else{
        $url = Url::fromRoute('wikis_grupo',array('group'=>$grupo_nid));
        $form_state->setRedirectUrl($url);
      }
    }
  }  
